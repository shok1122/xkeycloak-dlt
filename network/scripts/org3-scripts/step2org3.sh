...
set -x
peer channel fetch 0 $CHANNEL_NAME.block -o orderer.xkeycloak.com:7050 --ordererTLSHostnameOverride orderer.xkeycloak.com -c $CHANNEL_NAME --tls --cafile $ORDERER_CA >&log.txt
res=$?
set +x
cat log.txt
